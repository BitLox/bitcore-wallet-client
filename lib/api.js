...
API.prototype.seedFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPublicKey(xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.network, words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, {
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, network, cb) {
  var self = this;

  var privateKey = new Bitcore.PrivateKey(privateKey, network);
  var address = privateKey.publicKey.toAddress(network);
  self.getUtxos({
...
API.prototype.buildTxFromPrivateKey = function(privateKey, network, destinationAddress, opts, cb) {
  var self = this;
...
API.prototype._doPostRequest = function(url, args, cb) {
  args.network = 'livenet' // hardcode for now
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  ...
  return _.padRight(widBase58, 22, '0') + walletPrivKey.toWIF() + network;
};
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    if(networkChar === 'T') { networkChar = 'testnet' }
    if(networkChar === 'L') { networkChar = 'livenet' }
